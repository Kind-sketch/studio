...
'use server';
/**
 * @fileOverview A flow for translating text into different languages using the Cloud Translation API.
 *
...
import {z} from 'zod';
import { translateTexts as translate } from '@/services/translation-service';
...
const translateTextFlow = ai.defineFlow(
  ...
  async (input: TranslateTextInput) => {
    if (!process.env.GOOGLE_CLOUD_PROJECT || !process.env.GEMINI_API_KEY) {
      console.error('FATAL: GOOGLE_CLOUD_PROJECT or GEMINI_API_KEY is not configured. Translation will not work.');
    ...
    // If there are any texts that need translation, call the Translation API
    if (textsToTranslate.length > 0) {
      try {
        ...
        console.error("Cloud Translation API failed, using original texts as fallback:", error);
